...
from tomopyui.widgets.imports.Dxchange import Dxchange as DxchangeUploader
import time
...
class RawProjections_Dxchange(RawProjectionsBase):
    """
    ...
    def __init__(self):
        super().__init__()
        self.allowed_extensions = [".h5", ".hdf5"]
    ...
    def import_file_all(self, uploader: Optional[]=None):
        if uploader:
            self.import_status_label = uploader.import_status_label
        self.tic = time.perf_counter()
        
        self.filedir = pathlib.Path(uploader.filedir)
        self.filename = pathlib.Path(uploader.filename)
        ...
        self._data = dxchange.reader.read_hdf5(str(self.filepath))
