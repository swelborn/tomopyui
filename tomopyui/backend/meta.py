...
import numpy as np
import pandas as pd
from ipywidgets import Label, VBox, Layout, Output
from IPython.display import display
from tomopyui.backend.readers import get_metadata_instance
...
class Metadata(ABC):
    ...
    def __init__(self):
        self.table_label = Label(style=header_font_style)
        self.metadata = {}
        self.filedir: Optional[pathlib.Path] = pathlib.Path()
        self.filepath: Optional[pathlib.Path] = pathlib.Path()
    ...
    @staticmethod
    def parse_metadata_type(filepath: pathlib.Path, metadata=None) -> "Metadata":
        """
        ...
        # Scan types
        metadata_instance: Metadata = get_metadata_instance(metadata["metadata_type"])
    ...
    @staticmethod
    def create_metadatas(filepath: pathlib.Path) -> "list[Metadata]":
        """
...
class Metadata_MultiEnergy(Metadata):
    ...
    def make_init_metadata(self, folders):
        ...
        for folder in folders:
            ...
            metadatas = _prj.metadata.create_metadatas(_prj.metadata.filepath)
