...
import astra
from bqplot import LinearScale, Figure, ColorScale, Lines, Axis
import cupy as cp
import numpy as np
...
def initialize_reconstruction_dataset(prjs: np.ndarray) -> np.ndarray:
    tomo_shape = prjs.shape
    recon = np.mean(prjs) * np.empty(
        (tomo_shape[1], tomo_shape[2], tomo_shape[2]), dtype=np.float32
    )
    return recon


def setup_plots(run_align, projection_num: int) -> None:
    scale_x = LinearScale(min=0, max=1)
    scale_y = LinearScale(min=1, max=0)
    scales = {"x": scale_x, "y": scale_y}

    projection_fig = Figure(scales=scales)
    simulated_fig = Figure(scales=scales)

    scales_image = {
        "x": scale_x,
        "y": scale_y,
        "image": ColorScale(
            min=float(np.min(run_align.prjs[projection_num])),
            max=float(np.max(run_align.prjs[projection_num])),
            scheme="viridis",
        ),
    }

    image_projection = ImageGL(
        image=run_align.prjs[projection_num],
        scales=scales_image,
    )
    image_simulated = ImageGL(
        image=np.zeros_like(run_align.prjs[projection_num]),
        scales=scales_image,
    )
    ...
    with run_align.plot_output1:
        run_align.plot_output1.clear_output(wait=True)
        display(
            HBox(
    ...
    xs = LinearScale()
    ys = LinearScale()
    x = range(run_align.prjs.shape[0])
    y = [run_align.sx, run_align.sy]
    line = Lines(
        x=x,
        y=y,
        scales={"x": xs, "y": ys},
    ...
    xax = Axis(scale=xs, label="Projection Number", grid_lines="none")
    yax = Axis(
        scale=ys,
    ...
    fig_SxSy = Figure(marks=[line], axes=[xax, yax], animation_duration=1000)
    fig_SxSy.layout.width = "600px"
    # Initialize convergence plot
    xs_conv = LinearScale(min=0)
    ys_conv = LinearScale()
    x_conv = [0]
    y_conv = [run_align.conv[0]]
    line_conv = Lines(
        x=x_conv,
        y=y_conv,
    ...
    xax_conv = Axis(scale=xs_conv, label="Iteration", grid_lines="none")
    yax_conv = Axis(
        scale=ys_conv,
        orientation="vertical",
        tick_format="0.1f",
    ...
    with run_align.plot_output2:
        run_align.plot_output2.clear_output()
        display(
            HBox(
                [fig_SxSy, fig_conv],
                layout=Layout(
...
def reset_progress_bars(run_align):
    run_align.analysis_parent.progress_shifting.value = 0
    run_align.analysis_parent.progress_reprj.value = 0
    run_align.analysis_parent.progress_phase_cross_corr.value = 0


def perform_reconstruction(
    alignment_iter, run_align, recon_iterations, method_str, _rec, center
):
    # Define a mapping of method strings to their corresponding reconstruction functions
    reconstruction_methods = {
        "SIRT_Plugin": lambda: tomocupy_algorithm.recon_sirt_plugin(
            run_align.prjs,
            run_align.angles_rad,
            num_iter=recon_iterations,
            rec=_rec,
            center=center,
        ),
        "SIRT_3D": lambda: tomocupy_algorithm.recon_sirt_3D(
            run_align.prjs,
            run_align.angles_rad,
            num_iter=recon_iterations,
            rec=_rec,
        ...
        "CGLS_3D": lambda: tomocupy_algorithm.recon_cgls_3D_allgpu(
            run_align.prjs,
            run_align.angles_rad,
            num_iter=recon_iterations,
            rec=_rec,
    ...
    # Default case for methods not explicitly listed
    default_reconstruction = lambda: default_recon(
        alignment_iter, run_align, recon_iterations, method_str, _rec, center
    )

    # Execute the reconstruction function based on the method_str, using the default case if method_str is not found
    return reconstruction_methods.get(method_str, default_reconstruction)()
...
def perform_alignment(
    ...
    for alignment_iter in range(num_iter):
        recon_iterations = pre_alignment_iters if alignment_iter == 0 else 1
        # Your alignment logic here
        pass


def align_joint_new(run_align):
    os.environ["TOMOPY_PYTHON_THREADS"] = "1"

    # Unpacking and initialization
    prjs, num_iter, downsample = (
        run_align.prjs,
        run_align.num_iter,
        run_align.downsample,
    )
    method_str = list(run_align.metadata.metadata["methods"].keys())[0]
    method_str = "EM_CUDA" if method_str == "MLEM_CUDA" else method_str
    upsample_factor, num_batches, center, pre_alignment_iters = (
        run_align.upsample_factor,
        run_align.num_batches,
        run_align.center,
        run_align.pre_alignment_iters,
    )
    projection_num = 50  # Consider making this configurable

    # Operations
    prjs, scale_factor = scale_tomo(prjs)
    run_align.recon = initialize_reconstruction_dataset(prjs)
    setup_plots(prjs, projection_num)
    perform_alignment(
        run_align, num_iter, pre_alignment_iters, center, method_str, num_batches
    )

    # Final adjustments
    prjs *= scale_factor
    run_align.prjs = prjs
    run_align.recon = circ_mask(
        run_align.recon, axis=0
    )  # Assuming circ_mask is defined elsewhere
    if downsample:
        adjust_for_downsampling(run_align)  # Define this function based on your needs

    return run_align


def adjust_for_downsampling(run_align) -> None:
    # Adjust sx, sy, and shift for downsampling, similar to the last part of your original function
    pass


def align_joint(run_align):

    # ensure it only runs on 1 thread for CUDA
    os.environ["TOMOPY_PYTHON_THREADS"] = "1"

    # Initialize variables from metadata for ease of reading:
    num_iter = run_align.num_iter
    downsample = run_align.downsample
    method_str = list(run_align.metadata.metadata["methods"].keys())[0]
    if method_str == "MLEM_CUDA":
        method_str = "EM_CUDA"
    upsample_factor = run_align.upsample_factor
    num_batches = run_align.num_batches
    center = run_align.center
    pre_alignment_iters = run_align.pre_alignment_iters
    projection_num = 50  # default to 50 now, TODO: can make an option

    # Needs scaling for skimage float operations
    run_align.prjs, scl = scale_tomo(run_align.prjs)

    # Initialization of reconstruction dataset
    tomo_shape = run_align.prjs.shape
    run_align.recon = np.mean(run_align.prjs) * np.empty(
        (tomo_shape[1], tomo_shape[2], tomo_shape[2]), dtype=np.float32
    )

    # Initialize shift/convergence
    run_align.sx = np.zeros((tomo_shape[0]))
    run_align.sy = np.zeros((tomo_shape[0]))
    run_align.conv = np.zeros((num_iter))
    subset_x = run_align.subset_x
    subset_y = run_align.subset_y
    ...
    for n in range(num_iter):
        ...
        # for progress bars
        reset_progress_bars(run_align)
        _rec = run_align.recon
        # TODO: handle reconstruction-type parsing elsewhere
        if method_str == "SIRT_Plugin":
            run_align.recon = tomocupy_algorithm.recon_sirt_plugin(
                run_align.prjs,
                run_align.angles_rad,
                num_iter=recon_iterations,
                rec=_rec,
            ...
            run_align.recon = tomocupy_algorithm.recon_sirt_3D(
                run_align.prjs,
                run_align.angles_rad,
                num_iter=recon_iterations,
                rec=_rec,
            ...
            run_align.recon = tomocupy_algorithm.recon_cgls_3D_allgpu(
                run_align.prjs,
                run_align.angles_rad,
                num_iter=recon_iterations,
                rec=_rec,
            ...
            if n == 0:
                run_align.recon = tomopy_algorithm.recon(
                    run_align.prjs,
                    run_align.angles_rad,
                    algorithm=wrappers.astra,
                    center=center,
        ...
        run_align.recon[np.isnan(run_align.recon)] = 0
        run_align.analysis_parent.progress_total.value = n + 1
        # break up reconstruction into batches along z axis
        run_align.recon = np.array_split(run_align.recon, num_batches, axis=0)
        # may not need a copy.
        _rec = run_align.recon.copy()
        ...
        simulate_projections(
            ...
            center,
            run_align.angles_rad,
            progress=run_align.analysis_parent.progress_reprj,
        )
        ...
        batch_cross_correlation(
            ...
            num_batches,
            upsample_factor,
            subset_correlation=run_align.use_subset_correlation,
            subset_x=subset_x,
            subset_y=subset_y,
            blur=True,
            pad=run_align.pad_ds,
            progress=run_align.analysis_parent.progress_phase_cross_corr,
        )
        run_align.shift = np.concatenate(shift_cpu, axis=1)
        # Shifting.
        (
            run_align.prjs,
            run_align.sx,
            ...
            run_align.pad_ds,
            center,
            downsample_factor=run_align.ds_factor,
            progress=run_align.analysis_parent.progress_shifting,
        )
        run_align.conv[n] = np.linalg.norm(err)
        # update images
        image_projection.image = run_align.prjs[projection_num]
        image_simulated.image = sim[projection_num]
        # update plot lines
        line_conv.x = np.arange(0, n + 1)
        line_conv.y = run_align.conv[range(n + 1)]
        line.y = [
            run_align.sx * run_align.ds_factor,
            run_align.sy * run_align.ds_factor,
        ]
        run_align.recon = np.concatenate(run_align.recon, axis=0)
        mempool = cp.get_default_memory_pool()
        mempool.free_all_blocks()

    # Re-normalize data
    run_align.prjs *= scl
    run_align.recon = circ_mask(run_align.recon, 0)
    if downsample:
        run_align.sx *= run_align.ds_factor
        run_align.sy *= run_align.ds_factor
        run_align.shift *= run_align.ds_factor

    run_align.pad = tuple([int(x * run_align.ds_factor) for x in run_align.pad_ds])
    return run_align
...
def batch_cross_correlation(
    ...
    for batch in range(len(_prj)):
        ...
        if subset_correlation and subset_y and subset_x:
            _prj_gpu = cp.array(
...
def blur_edges_cp(prj, low=0.0, high=0.8):
    """
