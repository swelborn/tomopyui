...
def recon_dashboard(
    ...
    def radio_recon_true(change):
        if change.new == 0:
            ...
            reconmetadata["recondata"] = True
            reconmetadata["methods"]["SIRT_CUDA"] = {}
    ...
    def set_options_and_run_align(self):
        ...
        reconmetadata["opts"]["num_iter"] = number_of_recon_iterations.value
        reconmetadata["opts"]["center"] = center_of_rotation.value
        reconmetadata["opts"]["prj_range_x"] = projection_range_x_recon.value
        reconmetadata["opts"]["prj_range_y"] = projection_range_y_recon.value
        reconmetadata["opts"]["extra_options"] = extra_options.value
        #!!!!!!!!! what do these call backs do in recon.
        reconmetadata["callbacks"]["button"] = self
        reconmetadata["callbacks"]["methodoutput"] = method_output
        reconmetadata["callbacks"]["output0"] = output0
        reconmetadata["opts"]["downsample"] = downsample_checkbox.value
        reconmetadata["opts"]["downsample_factor"] = downsample_factor_text.value
        if len(reconmetadata["methods"]) > 1:
            reconmetadata["reconmultiple"] = True
        try:
            self.description = "Reconstructing your data."
            ##### reconstruction function goes here.
            # aligned_tomo_list.append(TomoAlign(tomo, reconmetadata))
            self.button_style = "success"
    ...
    def toggle_on(change, opt_list, dictname):
        if change.new == 1:
            reconmetadata["methods"][dictname] = {}
            for option in opt_list:
                option.disabled = False
        if change.new == 0:
            reconmetadata["methods"].pop(dictname)
            for option in opt_list:
                option.value = 0
    ...
    def create_dict_on_checkmark(change, opt_list, dictname):
        reconmetadata["methods"][dictname] = create_option_dictionary(opt_list)
    ...
    def downsample_turn_on(change):
        if change.new == 1:
            reconmetadata["opts"]["downsample"] = True
            downsample_factor_text.disabled = False
        if change.new == 0:
            reconmetadata["opts"]["downsample"] = False
    ...
    def create_save_dict_on_checkmark(change, opt_list):
        reconmetadata["save_opts"] = create_option_dictionary(opt_list)
    ...
    return recon_dashboard, plot_vbox, recon_files
